...
def compile_final_report() -> str:
    ...
    A(f"- **Stationarity risk:** the cointegration relationship is assessed over a {data_span} span "
      f"({data_summary['aligned_date_range'][0]} to {data_summary['aligned_date_range'][1]}); it may shift with "
      "seasonality or regime changes within or beyond that sample. The hedge ratio itself is a fixed contract spec, "
      "not an estimate, so it does not carry this risk, but the *validity* of trading it (stationarity, half-life) "
      "does. This pipeline does not automatically detect or adjust for contract rolls (MHI/HHI futures expire "
      "monthly) within a multi-month dataset; a manual review of this specific dataset (comparing daily close "
      "changes on and around each month's approximate expiry date, and checking whether the largest bar-to-bar "
      "jumps clustered near rollovers rather than elsewhere) found no evidence of roll-driven price discontinuities "
      "- the largest jumps instead line up with real macro news dates (e.g. the April 2025 tariff-driven selloff, "
      "visible as a ~7-9% single-day move in both legs) rather than expiry dates, and expiry-date-specific z-scores "
      "were statistically unremarkable (only 1 of 19 months exceeded |z|=2, with no consistent sign or magnitude). "
      "This is reassuring for the current dataset but was a one-off manual check, not an automated pipeline "
      "safeguard - it should be re-verified if the data window is extended or refreshed.")
    A("- **Data completeness:** the raw resample grid spans every calendar minute (24/7), so the headline \"bars "
      "with no trade\" figures in Section 2 look alarming (~54-58%) but are overwhelmingly explained by legitimate "
      "market closure - weekends, the daily 03:00-09:15 HKT gap between the night and day sessions, the 12:00-13:00 "
      "HKT lunch break, and Hong Kong public holidays (confirmed by checking that the longest no-trade runs are "
      "almost all exactly 165, 210, or 585 minutes - the exact lengths of a missing morning session, afternoon "
      "session, or full night session respectively, i.e. holiday closures, not feed gaps). Restricted to actual "
      "session hours only, the no-trade rate drops to ~10%, consistent with ordinary quiet-minute microstructure "
      "and modestly thinner liquidity in the small hours of the night session (~28-31% no-trade between 00:00-03:00 "
      "HKT vs. ~7-9% during the day session). Exactly one genuinely unexplained gap was found in the whole dataset "
      "(both legs stop simultaneously at 2026-06-05 23:41 HKT and do not resume until the normal Monday 09:15 HKT "
      "open) - its clean, synchronized-across-both-legs signature is more consistent with a real trading suspension "
      "(e.g. a typhoon-signal closure, common in Hong Kong) than a data vendor outage, but this was not independently "
      "confirmed against an external event calendar.")
